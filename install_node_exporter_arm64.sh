...
VER="1.9.1"  # Update this version number if a newer version is available
wget https://github.com/prometheus/node_exporter/releases/download/v${VER}/node_exporter-${VER}.linux-arm64.tar.gz
tar xvf node_exporter-${VER}.linux-arm64.tar.gz

# Move the binary and set permissions
sudo mv node_exporter-${VER}.linux-arm64/node_exporter /usr/local/bin/
...
rm -rf node_exporter-${VER}.linux-arm64*
